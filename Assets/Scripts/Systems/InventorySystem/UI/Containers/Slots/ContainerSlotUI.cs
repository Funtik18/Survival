...
public class ContainerSlotUI : MonoBehaviour 
{
	...
	private void ShowItemInformation()
    {
		...
		weightText.text = System.Math.Round(Data.CurrentWeight, 2) + "KG";
		ShowHideItemWeight(true);

		if (ScriptableData.isBreakable)
		{
			durabilityText.text = System.Math.Round(Data.CurrentDurrability, 2) + "%";
		...
		{
			countText.text = "x" + Data.StackSize;
